...
public class DatabaseController extends SQLController {
    ...
    public boolean createChart(int projectId, Layout.Chart chart, @Nullable String ownerId) throws SQLException {
        return executeUpdate(CREATE_CHART, projectId, chart.chart(), chart.name(), chart.type(), chart.staticInfo(),
                chart.dimensions().width(), chart.dimensions().height(), chart.position(), chart.icon(),
                Layout.Source.toJson(chart.sources()).toString(), chart.extras().toString(), ownerId, projectId) > 0;
    }
    ...
    public @Nullable Project getProject(String slug, @Nullable String owner) throws SQLException {
        ...
        return project != null ? project.withLayout(getLayout(project.id())).withData(getData(project.id())) : null;
    }

    private @Nullable ChartData getData(int projectId) throws SQLException {
        return null;
    }
    ...
    public boolean deleteChart(int projectId, String chart, @Nullable String ownerId) throws SQLException {
        return executeUpdate(DELETE_CHART, projectId, chart, ownerId) > 0;
    }
